...
void radio_generate_random (u1_t *buffer, u1_t len){
    while(len){
        ...
        memcpy(buffer, &randword, copy_len);
        buffer += copy_len;
        len -= copy_len;
    }
}
...
bool radio_irq_process (ostime_t irqtime, u1_t diomask) {
    ...
    if (isFsk(LMIC.rps)) { // FSK modem
        ...
        if (irqflags & IRQ_TXDONE) { // TXDONE
            ...
            //LMIC.rxtime0 = LMIC.rxtime - calcAirTime(LMIC.rps, LMIC.dataLen); // beginning of frame timestamp
#ifdef DEBUG_RX
            LMIC_DEBUG_PRINTF("RX[rssi=%d,snr=%.2F,len=%d]: %h\r\n",
            ...
            // unexpected irq
            LMIC_DEBUG_PRINTF("UNEXPECTED RADIO IRQ %04x\r\n", irqflags);
            ASSERT(0);
        }
    }
